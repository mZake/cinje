...
Finally, run `ninja` to build the code and inject it in a ROM. `base.gba` is used as the base ROM image, it must be present before building. When the build process finishes, the resulting ROM will be written to `out.gba`.
