...
def main():
    ...
    # Remove build file generated by the last execution to ensure it isn't used
    # when this execution doesn't generate one.
    if os.path.isfile(f"{PATCH_DIR}/build.ninja"):
        os.remove(f"{PATCH_DIR}/build.ninja")

    # Only generate a build file if there's targets to build
    if patch_sources:
